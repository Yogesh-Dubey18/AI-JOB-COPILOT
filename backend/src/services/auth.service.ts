...
function signTokens(user: any) {
  const accessToken = jwt.sign({ sub: String(user._id), role: user.role }, env.JWT_ACCESS_SECRET, { expiresIn: "15m" });
  const refreshToken = jwt.sign({ sub: String(user._id), tokenVersion: Date.now() }, env.JWT_REFRESH_SECRET, { expiresIn: "7d" });
