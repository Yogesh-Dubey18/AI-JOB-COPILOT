...
import { applicationInsights, createApplication, deleteApplication, getApplication, getApplicationTimeline, listApplications, scheduleFollowUp, updateApplication, updateApplicationStatus } from "../services/application.service.js";
...
router.get("/insights", asyncHandler(async (req, res) => res.json({ success: true, data: await applicationInsights(req.user!.id) })));
router.get("/:id", asyncHandler(async (req, res) => res.json({ success: true, data: await getApplication(req.user!.id, param(req.params.id)) })));
router.get("/:id/timeline", asyncHandler(async (req, res) => res.json({ success: true, data: await getApplicationTimeline(req.user!.id, param(req.params.id)) })));
router.patch("/:id/follow-up", asyncHandler(async (req, res) => res.json({ success: true, data: await scheduleFollowUp(req.user!.id, param(req.params.id), req.body.nextFollowUpDate, req.body.note) })));
